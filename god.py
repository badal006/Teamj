...
BOT_TOKEN = "7312643382:AAHbNJwjUZYd4bSruRaJspeOR4OGNzkCVEw"
OWNER_ID = 5879359815
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("WELCOME TO THE BOT!", parse_mode=ParseMode.MARKDOWN)
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Bot speed: {int(latency)} ms")
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.message.chat_id, text=f"BROADCAST:\n{message}")

async def destroy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) != 2:
        await update.message.reply_text("Usage: /destroy [ip] [port]")
        return
